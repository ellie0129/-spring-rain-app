...
if menu == "모형 설명":
    ...
    # sample_data.json 파일 경로 설정 (현재 파일과 같은 디렉토리에 위치)
    ...
    try:
        ...
        sample_profiles = sample_data.get("sample_profiles", {})
    ...
    if sample_profiles:
        selected_profile = st.selectbox("샘플 인물을 선택하세요", list(sample_profiles.keys()))
        
        st.markdown(f"### 🧾 {selected_profile}의 점수 프로파일")
        st.write(sample_profiles[selected_profile])
        st.markdown(f"**해석 주석**: {sample_comments.get(selected_profile, '해당 인물에 대한 설명이 없습니다.')}")
        
        # 단일 인물에 대한 레이더 차트 옵션
        if st.checkbox("🔍 점수 레이더 차트 보기"):
            profile_data = sample_profiles[selected_profile]
            labels = list(profile_data.keys())
        ...
        # 다중 인물 비교 차트 옵션
        if st.checkbox("📊 인물 비교 차트 보기"):
            selected_profiles_list = st.multiselect("비교할 인물을 선택하세요", list(sample_profiles.keys()), default=[selected_profile])
            if selected_profiles_list:
                fig = go.Figure()
                for name in selected_profiles_list:
                    data = sample_profiles[name]
                    labels = list(data.keys())
                    values = list(data.values())
                    labels.append(labels[0])
                    values.append(values[0])
